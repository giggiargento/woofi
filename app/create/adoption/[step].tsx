...
export default function CreateAdoptionCaseScreen() {
  ...
  const handlePublish = async () => {
    ...
    const input: CreateAdoptionCaseInput = {
      caseType: 'adoption',
  ...
  return (
    <>
      <Stack.Screen options={{ title: t('create.adoption') }} />
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 bg-background">
        <ScrollView className="flex-1 px-4 py-4" keyboardShouldPersistTaps="handled">
          ...
          <View className="mb-4 flex-row items-center justify-between">
            <Text className="text-text">{t('create.vaccinated')}</Text>
            <Switch value={vaccinated} onValueChange={setVaccinated} trackColor={{ true: '#CFF5DC' }} />
          </View>
          <View className="mb-4 flex-row items-center justify-between">
            <Text className="text-text">{t('create.neutered')}</Text>
            <Switch value={neutered} onValueChange={setNeutered} trackColor={{ true: '#CFF5DC' }} />
          </View>
